...
# --------------------------------------------------------------------- B10 rollout functional
def test_rollout_weighting_is_valid_when_present():
    """If B10 has run, its weighting must be a valid weighting: non-negative, finite, mean~1
    per covered cluster, and never all-zero on a cluster (the bug that crashed the first run)."""
    import glob
    from if_repair import functionals as F
    if not os.path.exists(F.WEIGHT_CACHE):
        pytest.skip("no weight cache")
    z = dict(np.load(F.WEIGHT_CACHE))
    if "rollout" not in z:
        pytest.skip("b10 not run")
    w = z["rollout"]
    assert np.isfinite(w).all() and (w >= 0).all()
    fm_import = __import__("if_repair.functionals", fromlist=["frame_meta"])
    fm = fm_import.frame_meta()
    for c in ("C1", "C5"):                       # the dev targets always have real clouds
        rows = fm["cluster_of_row"] == c
        assert w[rows].sum() > 0, f"{c} has zero total rollout weight"
