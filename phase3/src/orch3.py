"""Phase-3 orchestrator. A copy of src/orchestrator.py with two additions:

  * a job may name its TRAINER ("stock" = src/train.py, "factorial" = phase3 init/order split,
    "diffusion" = the P10 diffusion policy), so P8a/P10 reuse the same GPU-gating and resume logic;
  * the completion test is MARKER-GATED (P6.1): a job counts as done only when its eval marker
    exists -- never merely because outcomes.json is on disk.

HARD RULE (unchanged, never regressed): GPUs 4-7 only; a job launches only on a GPU that is idle
(mem < 1000 MiB AND util < 10%), verified immediately before EVERY launch; if none is idle, sleep
300 s and re-check.
"""
import argparse
import json
...
def job_cmd(job, gpu_id):
    ...
    if trainer == "factorial":
        tr = (f"{PY} {P3SRC}/train_factorial.py --run_dir {rd} --demos {dj} "
              f"--init_seed {job['init_seed']} --order_seed {job['order_seed']} {steps}")
    elif trainer == "diffusion":
    ...
    roll = (f"{P3SRC}/rollout_diffusion.py" if trainer == "diffusion"
            else f"{SRC}/rollout.py")
...
def prepare(job):
    ...
    if not os.path.exists(p):
        payload = {"demos": job["demos"]}
        for k in ("seed", "init_seed", "order_seed"):
            if k in job:
                payload[k] = job[k]
...
def run_jobs(jobs, stage, gpus=(4, 5, 6, 7), poll=10):
    logdir = os.path.join(P3_LOGS, stage)
        ...
        for g, (p, j, t0, lf) in list(running.items()):
            if p.poll() is not None:
                ...
                print(f"[{stage}] {'OK  ' if ok else 'FAIL'} "
                      ...
                      f"({n_done}/{len(jobs)-skipped})", flush=True)
                del running[g]
        ...
        if todo:
            ...
            for g in gpus:
                ...
                p = subprocess.Popen(job_cmd(j, g), shell=True, stdout=lf,
                                     stderr=subprocess.STDOUT, cwd=P3SRC)
    ...
    L.atomic_write_json(os.path.join(P3_LOGS, f"{stage}_summary.json"), summary)
